...
// Stałe do pomiarów
const int Ro = 14000; //Ro zmienna rezystancji dla czystego powietrza gdy f(PomiarTemp) = 1
const int RL = 9300; //Rezystor podpiętu między masą a pinem 2 czujnika Figaro
    ...
    case 8:
       while (digitalRead(ANULUJ) == LOW){
         ...
         do{
          ...
          u8g2.drawStr(20, 30, String(currentTemp).c_str());
...
float pomiar(int idPomiaru) {
  ...
  switch (idPomiaru){
    case 0:
      wynik = (pow((RS/Ro), -0.81) - 0.7) * PomiarTemp;
      break;
    case 1:
      break;
  }
...
void pomiarB(){
  if (digitalRead(BATERIA) == HIGH){
    if (bateriaDelay == 5){
      switch (stanBaterii){
        case 5:
          pomiarBaterii = ((analogRead(A2)) - 735) / 20;
    ...
    stanTemp = 0;
    pomiarBaterii = ((analogRead(A2)) - 700) / 20;
...
String tekstPomiaru(float dane, float minV, float maxV){
  ...
  int testPomiarowy = (dane - minV) / rozmiar * 2.9 - 0.1;
